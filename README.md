...
- 左下角以 `[1]`、`[World]`、`[Trade]` 等開頭的聊天行、網址與常見買賣廣告會在翻譯前丟棄，不再逐行翻譯。這是降低干擾的規則，不是隱私隔離；字幕框仍應避開聊天區。
- 任務面板每 12 個掃描週期讀一次，而且只有目標用途真的改變才處理；`2/5` 變成 `3/5` 這類進度數字不會重新觸發。
- 任務框不再先做逐字翻譯，而是顯示「用途：導航目標／推進對話／戰鬥目標／收集目標……」以及現在要採取的動作。誤掃到 `Click to stop tracking quests` 或地城出口提示時，也只解釋按鈕用途一次。
- 角色字幕採兩階段本機翻譯：規則／詞彙層先在畫面顯示繁中快速理解，再由 `qwen3.5:0.8b` 對照英文修正完整句子。翻譯列不重複顯示 OCR 英文，也不播放模型逐字生成的半句。
- 未知字幕需連續兩次 OCR 大致相同才會接手翻譯，避免單次辨識抖動一直取消上一句；已知短句仍可立即顯示。新字幕等待時只保留最新一筆，模型失敗也不會用原文覆蓋已顯示的繁中初稿。
...
- OCR 後先用本機規則篩掉聊天、交易、重複進度與裝飾文字。已知遊戲詞直接使用本機教材；其他需要理解的畫面文字先交給通常已暖機的 `qwen3.5:0.8b`。若 10 秒內沒有有效結果，只有在空閒的 2～4 thread 模式才升級到 `qwen3.5:2b-q4_K_M`，並限制背景判斷為 25 秒；忙碌的 1 thread 模式直接略過大模型。
- 大模型只處理小模型無法可靠完成的任務用途判斷、情境化教學與教材多樣化；翻譯、快取和固定攻略不等它。

## 英文、攻略與多益 500–750

教學先從目前兩個 OCR 區域真正出現的任務、按鈕或角色用語開始。教練先說它在遊戲中的用途，再隔幾段延伸同一個字在多益或數位 IC 的用法，不會突然插入無關課文。例如畫面出現 `confirm`，先理解 `Confirm your choice.`，之後才可能延伸 `Please confirm the delivery schedule.`。
...
確認 Ollama 正在執行且已安裝 `qwen3.5:0.8b`。本機規則會先給繁中快速理解，模型只做第二階段修正；第一次載入模型仍會較慢，常見片語和快取不需等待模型。字幕框越小、聊天越少，OCR 越快。
